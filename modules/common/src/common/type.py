...
class Piece:
    color: Color
    pos: int    # home: 0, track: 1-60, goal: 61-66
...
class BoardState:
    ...
    def __post_init__(self):
        ...
        if not (1 <= self.dice <= 6):
            raise ValueError(f"dice must be in range [1, 6], got {self.dice}")
...
class TrackCell:
    ...
    (yard: 0, track: 1-60 [60 == home_entry], home_stretch: 61-66, 6 cells).
    ...
    by one color: that color's pieces must land there exactly (pos == 60)
    ...
    shared_step: int | None = None  # 1..num_shared_steps, for kind == TRACK or HOME_ENTRY
    home_step: int | None = None    # 60 for HOME_ENTRY, 61-66 for HOME_STRETCH
    ...
    def __post_init__(self):
        ...
        if self.kind == CellKind.TRACK:
            ...
            if self.shared_step is None or self.shared_step < 1:
                raise ValueError(f"{self.id}: track cells need shared_step >= 1, got {self.shared_step}")
            ...
            if self.shared_step is None or self.shared_step < 1:
                raise ValueError(f"{self.id}: home_entry cells need shared_step >= 1, got {self.shared_step}")
